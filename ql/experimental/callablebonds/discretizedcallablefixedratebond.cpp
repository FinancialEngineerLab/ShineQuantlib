...
namespace QuantLib {
    ...
    : arguments_(args) {
        ...
        callabilityPrices_ = args.callabilityPrices;

        // To avoid mispricing, we snap exercise dates to the closest coupon date.
        // This might require also changing the corresponding exercise price.
        for (Size i=0; i<callabilityTimes_.size(); ++i) {
            Time& exerciseTime = callabilityTimes_[i];
            for (Time couponTime : couponTimes_) {
                if (withinNextWeek(exerciseTime, couponTime)) {
                    exerciseTime = couponTime;
                    callabilityPrices_[i] -= args.callabilityAccruals[i];
                    break;
                }
            }
        }
        ...
        switch (arguments_.putCallSchedule[i]->type() ) {
          case Callability::Call:
            for (j=0; j<values_.size(); j++) {
                values_[j] = std::min(callabilityPrices_[i], values_[j]);
            }
            break;
          case Callability::Put:
            for (j=0; j<values_.size(); j++) {
                values_[j] = std::max(values_[j], callabilityPrices_[i]);
            }
            break;
          default:
